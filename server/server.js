  ...
  try {
    ...
    const pdfBuffer = await new Promise(async (resolve, reject) => {
      try {
        ...
        // FIXED: Ensure there's enough space for summary box
        if (boxY > 450) {
          doc.addPage({ size: "A4", layout: "landscape", margin: 40 });
          boxY = 40;
  ...
  try {
    ...
    const pdfBuffer = await new Promise(async (resolve, reject) => {
      try {
        ...
        doc.fontSize(9).font('Helvetica')
           .text(`This document is not subject to Sales & Service Tax (SST).`, 
                 0, 750, { align: 'center', width: doc.page.width });
        
        doc.text(`Generated by ${company.name} Inventory System`, 
                 0, 765, { align: 'center', width: doc.page.width });
...
// ============================================================================
//                    UPDATED: SINGLE PAGE PURCHASE INVOICE PDF
// ============================================================================
...
// ============================================================================
//                    UPDATED: SINGLE PAGE SALES INVOICE PDF
// ============================================================================
...
function generateInvoicePDFBuffer({ title = 'Invoice', companyInfo = {}, docMeta = {}, customer = {}, items = [], totals = {}, extraNotes = '' }) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fontSize(9).font('Helvetica')
         .text(`This invoice is not subject to Sales & Service Tax (SST).`, 
               0, 760, { align: 'center', width: doc.page.width });
