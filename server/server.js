...
// ============================================================================
//         FINAL STABLE VERSION - FIXED ALIGNMENT & PAGE ENFORCEMENT
// ============================================================================
  ...
  try {
    ...
    const pdfBuffer = await new Promise((resolve, reject) => {
      ...
      const drawHeader = () => {
        ...
        // We use absolute Y coordinates for these lines to ensure they stay level
        doc.text(company.address, 40, 65, { width: 400 }); 
        doc.text(`Phone: ${company.phone}`, 40, 85); 
        doc.text(`Email: ${company.email}`, 40, 97);
      ...
      items.forEach((item, index) => {
        // Reduced break limit to 440 to ensure footer fits on Page 1
        if (currentY > 440) {
        ...
        if (qty === 0) {
          ...
          doc.fillColor("#FF8C00").text("Low Stock", col.status + 5, currentY + 7);
        } else {
      ...
      doc.text(`${items.length}`, summaryX + 10, summaryY + 28, { width: summaryWidth - 20, align: 'right' });
      
      doc.text(`Total Quantity:`, summaryX + 10, summaryY + 43);
      doc.text(`${subtotalQty} units`, summaryX + 10, summaryY + 43, { width: summaryWidth - 20, align: 'right' });
      ...
      for (let i = 0; i < pages.count; i++) {
        ...
        // We use absolute coordinates (530 instead of relative bottom) to prevent Page 2
        doc.text(`Generated by ${company.name} Inventory System`, 0, 530, { align: "center", width: 842 });
        doc.text(`Page ${i + 1} of ${pages.count}`, 0, 545, { align: "center", width: 842 });
      }
