...
async function confirmAndAddProduct(){
  ...
  if(!sku || !name) {
    alert('⚠️ Please enter SKU and Name.');
    return;
  }
  ...
  try {
    ...
    if(res.ok) {
      // Clear form
      ['#p_sku','#p_name','#p_category','#p_quantity','#p_unitCost','#p_unitPrice'].forEach(id => { 
...
// Calculate total revenue for sales
...
function checkStockAvailability() {
  ...
  if (!productSelect?.value || !warningElement) return;
...
async function bindProductPage(){
  ...
  qs('#saveProductBtn')?.addEventListener('click', async ()=> {
    ...
    try {
      const res = await apiFetch(`${API_BASE}/inventory/${idVal}`, { 
        method: 'PUT', 
  ...
  qs('#changePasswordBtn')?.addEventListener('click', async ()=> {
    ...
    try {
      const res = await apiFetch(`${API_BASE}/account/password`, { 
        method: 'PUT', 
        body: JSON.stringify({ username: currentUsername, newPassword: newPass, securityCode: code }) 
      });
      const data = await res.json();
...
document.addEventListener('DOMContentLoaded', ()=> {
  if(currentPage.includes('login.html')) {
    ...
    if (qs('#contactPhone') && window.CONFIG && CONFIG.CONTACT_PHONE) {
      qs('#contactPhone').textContent = CONFIG.CONTACT_PHONE;
    }
  }
});
